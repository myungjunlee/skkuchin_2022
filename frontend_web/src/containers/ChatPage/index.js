...
const ChatPage = (props) => {
    ...
    useEffect(() => {
        ...
        if (auth.user && user_uid) {
            ...
            un_subscribe = dispatch(getRealtimeUser({ uid_1: auth.user.uid, uid_2: user_uid }))
            .then(un_subscribe => {
                return un_subscribe;
            })
            .catch(error => {
            ...
            unsubscribe = dispatch(getRealtimeConversations({ uid_1: auth.user.uid, uid_2: user_uid }))
            .then(unsubscribe => {
                return unsubscribe;
            })
            .catch(error => {
    ...
    useEffect(() => {
        return () => {
            ...
            dispatch(logoutChat());
            dispatch(logoutPost());
            un_subscribe.then(f => f()).catch(error => console.log(error));
    ...
    return (
        <Layout>
                ...
                <div className="chatpage_footer">
                    {
                        ...
                        <div className="chatpage_footer2">
                            ...
                            <div className="chatpage_messageBtn">
                                <img src={chat_send_button} width="44px" width="44px" onClick={submitMessage}/>
                            </div>
                        </div>
                    }
                </div>
            ...
            { exitPopup ? <Popup popupOff={closeExitPopup} popupFunc={exit} popupContent='채팅방을 나갈 시 채팅 목록 및 내화 내용이 삭제되고 복구할 수 없습니다. &#13;&#10;채팅방에서 나가시겠습니까?' popupOk='나가기' popupCancel='취소'  /> : null }
